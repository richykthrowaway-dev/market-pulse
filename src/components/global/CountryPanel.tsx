...
export default function CountryPanel({
  ...
  // PERF: defensive active-tab tracking.  Radix Tabs primitive unmounts
  // inactive content by default, but we double-gate by explicitly returning
  // null for non-active tab children.  This:
  //   - guards against any future shadcn/Radix change that forceMounts
  //   - makes the lazy-mount behaviour explicit and reviewable
  //   - prevents iframe-heavy tabs (Screener / Economy) from racing their
  //     mount work the instant a country is clicked
  ...
  return (
    <div className="h-full flex flex-col animate-in slide-in-from-right-5 fade-in duration-300">
      ...
      {/* Tabs */}
      <Tabs defaultValue="summary" value={activeTab} onValueChange={handleTabChange} className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Each tab body is double-gated: Radix unmounts inactive content,
            AND we explicitly only render the child when activeTab matches.
            This guarantees iframe-heavy / hook-heavy components don't fire
            their work on country click unless the user actually opens
            that tab. */}
        ...
        <TabsContent value="news" className="flex-1 overflow-y-auto px-4 pb-4">
          {activeTab === 'news' && <CountryNews iso2={iso2} />}
        </TabsContent>
        <TabsContent value="screener" className="flex-1 overflow-y-auto px-4 pb-4">
          {activeTab === 'screener' && <CountryScreener iso2={iso2} />}
        </TabsContent>
        ...
        <TabsContent value="economy" className="flex-1 overflow-y-auto px-4 pb-4">
          {activeTab === 'economy' && <CountryEconomy iso2={iso2} />}
        </TabsContent>
